...
void main() {
    ...
    float alpha_sum = grass_texture_alpha + forest_texture_alpha + tree_stone_texture_alpha + granite_texture_alpha + snow_texture_alpha;
    ...
    vec3 final_texture = vec3(grass_texture * grass_texture_alpha) + vec3(forest_texture * forest_texture_alpha) + vec3(tree_stone_texture * tree_stone_texture_alpha) + vec3(granite_texture * granite_texture_alpha) + vec3(snow_texture * snow_texture_alpha);

    if (pos[2] < 23) {
        fr_color = vec3(min(max((ambient + diffuse + water_texture) * (sin(0.5 * (water_color + pos[1])) / 10 + 0.9) + specular, 0.0), 1.0));
    }
    else {
        fr_color = vec3(min(max((ambient + diffuse + final_texture) * obj_color + specular, 0.0), 1.0));
    }
}